...
class AllIncluded_Trainer(LOGAN_NGD_Trainer):
    def __init__(self,
                 model: SimpleGAN, dataloader,
                 lr=0.00005, clip_value=0.01, gp_weight=10.0,
                 discriminator_steps=5,
                 ...
                 save_freq=10,
                 log_dir=None):
        super(AllIncluded_Trainer, self).__init__(model, dataloader, lr, clip_value, gp_weight, discriminator_steps,
                                          files_to_backup, save_freq, log_dir)
...
class GTTrainer(Trainer):
    """
    Copying everything from the official repo
    """
    def __init__(self,
                 model: SimpleGAN, dataloader,
    ...
    def step_discriminator(self, data, iteration):
        ...
        # no backprop to gen
        fake = self.model.generator.generate_batch(batch_size=batch_size).detach()

        fake_pred = self.model.discriminator(fake)
        real_pred = self.model.discriminator(data)

        gradient_penalty = self.gradient_penalty(data, fake)
        ...
        self.opt_discriminator.step()
        self.report_running_discriminator(real=data, losses=(loss_true.item(), loss_fake.item(), gradient_penalty),
                                          iteration=iteration)
    ...
    def step_generator(self, iteration):
        ...
        self.opt_generator.step()
        self.report_running_generator(fake=fake_data.detach(), loss=loss_g.item(), iteration=iteration)
        return loss_g.item()
