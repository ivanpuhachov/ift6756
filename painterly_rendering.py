...
def main(args):
    ...
    # target = torch.from_numpy(image).float().cuda() / 255.0
    ...
    img = render(canvas_width,  # width
                 ...
                 None,
                 *scene_args)
    pydiffvg.imwrite(img.cpu(), 'logs/painterly_rendering/init.png', gamma=gamma)
    ...
    for t in range(args.num_iter):
        ...
        pydiffvg.imwrite(img.cpu(), 'logs/painterly_rendering/iter_{}.png'.format(t), gamma=gamma)
        img = img[:, :, :3]
        ...
        if t % 10 == 0 or t == args.num_iter - 1:
            ...
            plt.show()
            # pydiffvg.save_svg('logs/painterly_rendering/iter_{}.svg'.format(t),
            #                   canvas_width, canvas_height, shapes, shape_groups)
    # pydiffvg.save_svg('data/lion.svg',
    #                   canvas_width, canvas_height, shapes, shape_groups)
    ...
    img = render(target.shape[1],  # width
                 ...
                 *scene_args)
    # Save the intermediate render.
    # pydiffvg.imwrite(img.cpu(), 'logs/painterly_rendering/final.png'.format(t), gamma=gamma)
    # Convert the intermediate renderings to a video.


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--target", help="target image path", default = 'data/lion3.jpg')
